...
function formulaires_cheque_charger_dist($options = array()){
...
function formulaires_cheque_traiter_dist($options = array()){ 
  ...
  if($action = charger_fonction('instituer_commande', 'action',true)) {
        ...
        sql_updateq('spip_commandes',array('type_paiement'=>'cheque'),'id_commande='.$id_commande);
   ...
   $valeurs['message_ok'].= _T('shop_paiements:explication_cheque').'<br/>';
   ...
   $valeurs['message_ok'].= recuperer_fond('inclure/message_paiement_cheque');   
